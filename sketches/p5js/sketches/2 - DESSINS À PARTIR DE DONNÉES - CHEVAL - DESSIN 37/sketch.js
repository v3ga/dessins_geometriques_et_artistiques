...
let A=0,B=0,B1=0,X=0,Y=0,X_=0,Y_=0;
// ----------------------------------------------------
function setup() 
{
  INIT();
  ...
  for (let I=0;I<=15;I++)
  {
    ...
    while(true)
    {
      ...
      B = READ();
      let AN=2*I*PI/6+PI/12,CO=Math.cos(AN),SI=Math.sin(AN),R=pow(.87,I);
      X_=.15+A/110,Y_=.15+B/110;
      X=int(NP*(.5+R*(CO*X_-SI*Y_)))
      Y=int(NP*(.5+R*(SI*X_+CO*Y_)));
